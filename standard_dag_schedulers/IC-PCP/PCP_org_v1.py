...
def getCheapestAssignment( p ) :
    ...
    if verbose>0 :
        d_childs = ""
        for i in range(0, len(dominant_childs) ):
            d_childs += " "+G.nodes[dominant_childs[i]]["name"]+":"+str(lft_limits[i])
        print("lft_limits dominant childs:"+d_childs)

        d_parents = ""
        for i in range(0, len(dominant_parents) ):
            d_parents += " "+G.nodes[dominant_parents[i]]["name"]+":"+str(est_limits[i])
        print("est_limits dominant parents:"+d_parents)

    new_best_cas  = 0
    new_best_cost = 0
    new_instance_time = 0 # NEW-IMPL seems to be missing in the original implementation (ERROR in line 963)
    # OLD-IMPL
...
def determineClusterLimits( p, q ) :
    ...
    for i in p:
        ...
        if num_children == 0 :
            # i has no children
            lft_limits.append( G.nodes[i]["LFT"] )
            dominant_childs.append( i )
            #print "  a) lft_limits <-","t"+str(i)+"("+str( G.nodes[i]["LFT"] )+")"
        else:              
            if min_lft_p<2*deadline :
                ...
                lft_limits.append( G.nodes[ p[len(p)-1] ]["LFT"] )
                dominant_childs.append( p[len(p)-1] )
                #print "  c) lft_limits <-","t"+str(G.nodes[ p[len(p)-1] ]["name"])+"("+str( G.nodes[ p[len(p)-1] ]["LFT"] )+")"                                  
        ...
        if num_parents == 0 :
            # i has no parents
            est_limits.append( G.nodes[i]["EST"] )
            ...
            if max_est_p > 0 :
                ...
                est_limits.append( G.nodes[ p[0] ]["EST"] )
...
def checkClusterLimits( l, p ) :
    ...
    for i in range(0,len(p)) :
        ...
        if verbose>0 :
            print("R2 p["+str(i)+"]="+G.nodes[p[i]]["name"]+": (p[0].EST="+str(G.nodes[p[0]]["EST"])+"+inst_time("+str(l)+")="+str(incr_inst_time)+") < est_limits["+str(i)+"]="+str(est_limits[i])+"(parent "+G.nodes[dominant_parents[i]]["name"]+")?", end=' ')
    ...
    for i in range(0,len(p)) :
        j = len(p)-1-i
        if verbose>0 :
            print("R3 p["+str(j)+"]="+G.nodes[p[j]]["name"]+": lft_p="+str(lft_p)+"-inst_time("+str(l)+")="+str(incr_inst_time)+" >lft_limits["+str(j)+"]="+str(lft_limits[j])+"(child "+G.nodes[dominant_childs[j]]["name"]+")?", end=' ')
...
def inCluster( p, cluster ) :
    ...
    if len(cluster)>0 :
        for i in cluster :
            if G.nodes[p]["name"] == G.nodes[i]["name"] :
                return True           
    return False
